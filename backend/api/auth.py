...
ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", 1000000)) # 여기 숫자 개발 완료 시 30으로 수정
...
async def kakao_login(
    token_request: schemas.KakaoLoginRequest,
    db: AsyncSession = Depends(get_db)
):
    # 1. 카카오 서버에 액세스 토큰을 보내 사용자 정보 요청
    kakao_access_token = token_request.access_token
    ...
    query = select(models.User).where(models.User.login_id == str(kakao_id))
